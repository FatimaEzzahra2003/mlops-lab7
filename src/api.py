...
Ce service :
- charge dynamiquement le modèle courant depuis MLflow (alias production);
...
def load_model_if_needed() -> tuple[str, Any]:
    """
    Charge le modèle actif depuis MLflow si le cache est vide ou différent.
    """
    mlflow.set_tracking_uri(MLFLOW_TRACKING_URI)
    cache_key = MODEL_URI
